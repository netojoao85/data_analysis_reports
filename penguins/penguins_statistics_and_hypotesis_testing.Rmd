...
![](images/lter_penguins.png){width="197"}
:::
...
<style>
 ...
 /*--- HEADER & TITLE -------------------------------------------------*/
      ...
      div#header {background-color:none;margin-top:0px}
      
      h1.title{ padding: 20px 0px 0px 0px;
                margin-top:0px;
                margin-left:250px;
                color:black;}
      
      h3.subtitle{margin-left:250px;margin-top:0px}
  
  
  
  /*--- TOC_FLOAT ------------------------------------------------------*/
  
      /* -- Costumise tab's color selected --*/
      li.tocify-item.list-group-item.active {background-color: #0F6F74;}
  ...
  /*--- BODY -----------------------------------------------------------*/
      ...
      body {background-color: #FFFCF5; color: "black";}
            
      p.body {text-align:justify; line-height: 3.5;}
  ...
  /*--- TABSETS --------------------------------------------------------*/            
      ...
      /*-- Dimension of each tab --*/
      .tabset .nav-tabs > li {width: 50%;}
      ...
      /*-- Tab NO selected --*/ 
      .nav-tabs>li>a {background-color:#4C8799; color:white;}
      
      /*-- Tab NO selected hover --*/ 
      .nav-tabs>li>a:hover {background-color:#4C8799; color:#EC8E2A;}
      
      /*-- border --*/
      .nav-tabs {border-bottom: 1px solid #343D72;}
...
p <- penguins %>% 
  ...
  theme(
    ...
    panel.background = element_rect("#F5F2EB"),
    panel.border = element_blank()
  )

# Apply different strip backgrounds using facet_theme from ggh4x
p + ggh4x::facet_wrap2(~species,  scales = "free", nrow = 3,
                strip = strip_themed(
...
-   **Adelie:** the distribution of bill length is symetric; there are not outliers.

-   **Chinstrap:** the distribution is assymetric; negatively skewed (skewed left).
...
bloxpot_summary <- as.tibble(penguins %>% 
  group_by(species) %>% 
  summarise(
